...
pygame.display.set_caption("打砖块游戏-V1.3")
...
# 创建游戏元素实例
paddle = Paddle(width // 2 - 75, height - 40, 150, 10)  # 增大球拍宽度
...
total_attempts = 0
total_score = 0
total_game_duration = 0  # 总游戏时长，单位：秒
episode_scores = []
game_start_time = None
game_duration = 0
total_training_start_time = None  # 新增：总训练开始时间

# 在全局变量区域添加失败计数器
failed_attempts = 0  # 新增失败尝试计数器
max_failed_attempts = 50  # 新增最大允许失败次数
...
def reset_game():
    global paddle, ball, bricks, game_start_time, game_duration, consecutive_hits, no_action_counter, level, total_game_duration
    # 重置关卡为 1
    level = 1
    paddle = Paddle(width // 2 - 75, height - 40, 150, 10)  # 增大球拍宽度
    ball = Ball(width // 2, height // 2, 10)
    ...
    while True:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                if total_training_start_time:
                    total_game_duration += time.time() - total_training_start_time
                pygame.quit()
                quit()
        ...
        if not running:
            ...
            draw_button("开始游戏", width//2 - btn_width//2, height//2 - 50, 
                        ...
                        lambda: [globals().update({'running': True, 'game_start_time': time.time(), 'total_training_start_time': time.time()})])
            ...
            if game_over:
                if total_training_start_time:
                    total_game_duration += time.time() - total_training_start_time
                    total_training_start_time = None
                # 修改游戏结束菜单按钮
                ...
                draw_button("重新开始", width // 2 - btn_width // 2, height // 2 - 50,
                            ...
                            lambda: [globals().update({'running': False, 'game_over': False, 'paused': False}), reset_game(),
                                     globals().update({'running': True, 'game_start_time': time.time(), 'total_training_start_time': time.time(),
                ...
                if paused:
                    ...
                    draw_button("继续", width - 140, 70, btn_width, btn_height, GRAY, LIGHT_GRAY, 
                                lambda: [globals().update({'paused': False}), globals().update({'total_training_start_time': time.time()})])
                    ...
                    if not total_training_start_time:
                    ...
                    hours = int(total_game_duration // 3600)
                    minutes = int((total_game_duration % 3600) // 60)
                    seconds = int(total_game_duration % 60)
                    if total_training_start_time:
                        current_duration = time.time() - total_training_start_time
                        total_hours = int((total_game_duration + current_duration) // 3600)
                        total_minutes = int(((total_game_duration + current_duration) % 3600) // 60)
                        total_seconds = int((total_game_duration + current_duration) % 60)
                        time_str = f"{total_hours:02d}:{total_minutes:02d}:{total_seconds:02d}"
                    else:
                        time_str = f"{hours:02d}:{minutes:02d}:{seconds:02d}"
                    status_line = f"关卡:{level}  次数:{len(episode_scores)+1}  时间:{time_str}  得分:{total_score}"
                    screen.blit(stats_font.render(status_line, True, WHITE), (20, stat_y))
                    ...
                    if not paused:
                        if game_start_time is not None:
                            ...
                            if random.random() < epsilon:
                                # 确保向左、不动、向右移动的权重相同
                                action = random.choice([0, 1, 2])  # 0:左移, 1:不动, 2:右移
                            else:
