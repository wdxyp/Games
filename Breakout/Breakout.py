...
# 创建游戏元素实例
paddle = Paddle(width // 2 - 50, height - 20, 100, 10)
...
# 重置游戏状态
def reset_game():
    global paddle, ball, bricks, game_start_time, game_duration
    paddle = Paddle(width // 2 - 50, height - 20, 100, 10)
    ball = Ball(width // 2, height // 2, 10)
    bricks = []
    brick_width, brick_height = 80, 30
        ...
        if not running:
            ...
            if game_over:
                ...
                if paused:
                    ...
                    # 碰撞检测
                    reward = check_ball_paddle_collision(ball, paddle)
                    reward += check_ball_brick_collision(ball, bricks)
                    total_score += reward
                    ...
                    #    consecutive_hits += 1
                    #   reward = 10 + consecutive_hits * 2  # 连续击中奖励
                    #else:
                    #    consecutive_hits = 0

                    #brick_reward = check_ball_brick_collision(ball, bricks)
                    #if brick_reward > 0:
                    #    consecutive_hits += 1
                    #    reward += brick_reward + consecutive_hits * 2  # 连续击中奖励///
                    ...
                    # 更新目标网络
                    step_count = 0
                    target_update_freq = 50  # 更频繁地更新目标网络
                    step_count += 1
